...
void quicksortRec (int esq, int dir) {
    int i = esq, j = dir;
    Personagens pivo = lista.array[(dir+esq)/2];
    while (i <= j) {
        incrementarContador(1);
        while (maior(pivo,lista.array[i])) i++;incrementarContador(1);
        while (maior(lista.array[j],pivo)) j--;incrementarContador(1);
        if (i <= j) {
        swap(i, j);
            i++;
            j--;
...
int main()
{
    ...
    quicksortRec(0,lista.n-1);
    mostrar2();


    FILE *pont_arq;
    pont_arq = fopen("659318_quicksort.txt", "w");
    Ticks[1] = clock();
