...
bool maior(Personagens a, Personagens b){
    bool res = false;
    if(a.peso > b.peso) res = true;
    else if(a.peso == b.peso && diferenca(a.nome,b.nome) > 0) res = true;
    return res;
}
...
{ 
    ...
    { 
        ...
        { 
            Personagens temp = lista.array[i]; 
            int j;             
            for (j = i; j >= gap && maior(lista.array[j - gap], temp); j -= gap){
                lista.array[j] =  lista.array[j - gap];
...
int main()
{
    ...
    FILE *pont_arq;
    pont_arq = fopen("659318_shellsort.txt", "w");
    Ticks[1] = clock();
