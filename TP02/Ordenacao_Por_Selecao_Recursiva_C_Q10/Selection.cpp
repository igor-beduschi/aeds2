...
void selectionSort(int i, int n)
{
	int min = i;
	for (int j = i + 1; j < n; j++)
	{
        incrementarContador(2);
		if(diferenca(lista.array[j].nome , lista.array[min].nome) < 0 )
			min = j;
	}
...
int main()
{
    ...
    selectionSort(0,lista.n);
    mostrar2();


    FILE *pont_arq;
    pont_arq = fopen("659318_selecaoRecursiva.txt", "w");
    Ticks[1] = clock();
