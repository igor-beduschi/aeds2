...
void mostrarAoContrario(Celula* atual,int entradas){
  if(atual->prox != NULL){
    mostrarAoContrario(atual->prox,entradas-1);
  }
  printf("[%d] ",entradas);
  PrintarObjeto(atual->elemento);
}

void mostrar(int entradas){
  mostrarAoContrario(topo,entradas);
}
